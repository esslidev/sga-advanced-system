export enum FrSuccessTitle {
  SIGNED_IN = "Connexion réussie",
  SIGNED_UP = "Inscription réussie",
  SIGNED_OUT = "Déconnexion réussie",
  // Visitor-related
  VISITOR_CREATED = "Le visiteur a été créé avec succès",
  VISITOR_UPDATED = "Les informations du visiteur ont été mises à jour avec succès",
  VISITOR_DELETED = "Le visiteur a été supprimé avec succès",
  // Visit-related
  VISIT_REGISTERED = "La visite a été enregistrée avec succès",
  VISIT_UPDATED = "Visite mise à jour",
  VISIT_DELETED = "Visite supprimée",
}

export enum FrSuccessMessage {
  SIGNED_IN = "L'utilisateur s'est connecté au système avec succès.",
  SIGNED_UP = "Le compte utilisateur a été créé avec succès dans le système.",
  SIGNED_OUT = "L'utilisateur a été déconnecté du système avec succès.",
  // Visitor-related
  VISITOR_CREATED = "Le visiteur a été créé dans le système avec succès.",
  VISITOR_UPDATED = "Les informations du visiteur ont été mises à jour avec succès.",
  VISITOR_DELETED = "Le visiteur a été supprimé du système avec succès.",
  // Visit-related
  VISIT_REGISTERED = "La visite a été enregistrée avec succès.",
  VISIT_UPDATED = "Les informations de la visite ont été mises à jour dans le système avec succès.", // ✅ Added
  VISIT_DELETED = "La visite a été supprimée du système avec succès.",
}
